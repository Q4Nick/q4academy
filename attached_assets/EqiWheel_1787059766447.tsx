 ...
 *
 * Overgenomen uit src/components/EqiWheel.tsx van de Q4-website; alleen de imports zijn
...
import mhsLogo from "./mhs-logo.png";
import {
  CENTER,
  COMPOSITE_GEOMETRY,
  COMPOSITE_TRACKING,
  ...
  RIM_FONT_SIZE,
  RIM_LABELS,
  RIM_LETTER_SPACING_EM,
  VIEWBOX,
  WB_FONT_SIZE,
  WB_LETTER_SPACING_EM,
  WELLBEING_LABELS,
  compositeFontSize,
  subLabelLayouts,
  subNames,
...
export function EqiWheel({
  ...
  intro?: string | null;
  /**
   * Override the operating tip under the wheel. The built-in text talks about
   * the mouse and the Tab key, which is nonsense on a phone, so the mobile
   * tree passes its own line (pass null to drop it entirely).
   */
  tip?: string | null;
  /**
  ...
  const tipText = tip === undefined ? text.tip : tip;
  ...
  const accent =
    selectedComposite?.color ??
    (selected === HEART_ID ? EQI_CORE : EQI_NEUTRAL);
  ...
  return (
    ...
    >
      ...
      <div className={wide ? "lg:mx-auto lg:max-w-3xl lg:px-8" : ""}>
        ...
        {/* Legend doubles as a keyboard-friendly way to pick a scale. The five
            chips are meant to read as one row of equals, so they stay on a
            single line; the sizing below is tuned to make the longest labels
            in both languages fit the wheel's column. */}
        {/* On the desktop page the five chips read as one row of equals, so
            they stay on a single line. In the phone shell that line cannot
            fit; there they wrap instead of sliding off the screen. */}
        <ul
          className={`mt-5 flex gap-1.5 list-none p-0 lg:mt-4 ${
            wide ? "flex-nowrap" : "flex-wrap justify-center"
          }`}
        >
            ...
            return (
              <li key={comp.id} className="m-0 p-0">
                <button
                  ...
                  className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border-[1.5px] bg-white px-3 py-1.5 text-[13px] font-semibold text-slate-700 transition-all hover:-translate-y-px hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2"
                  style={{
      ...
      <div
        className={`mt-5 grid gap-6 lg:mt-4 ${
          wide
            ? "lg:grid-cols-[minmax(0,2.5fr)_minmax(330px,1fr)] lg:items-stretch"
            : ""
        }`}
      >
        {/* ---------------- wheel ---------------- */}
        ...
        <div className="flex h-full flex-col">
          {/* h-full + flex: the card grows to the height of the panel beside
              it, and the operating tip sits inside the card, so both end on
              exactly the same line. */}
          <div className="flex h-full flex-col gap-3 rounded-3xl border border-slate-200 bg-white p-2 shadow-sm">
            <div className="relative flex flex-1 items-center justify-center">
              {/* MHS publishes the EQ-i 2.0; the mark sits in the empty corner
                beside the wheel, where it credits the model without competing
                with it. On a phone the wheel fills the whole card, so the mark
                has to shrink and tuck into the corner to stay off the wheel. */}
              <img
                src={mhsLogo}
                alt="MHS — Multi-Health Systems"
                width={309}
                height={120}
                className={`pointer-events-none absolute h-auto opacity-80 select-none ${
                  wide
                    ? "bottom-4 left-5 w-[92px]"
                    : "bottom-1.5 left-2 w-[52px]"
                }`}
              />
              <svg
              ...
              >
                  ...
                  {RIM_LABELS.map((a) => (
                    <ArcText
                      ...
                      style={{
                        fill: "#1c4a91",
                        fontWeight: 700,
                        letterSpacing: `${RIM_LETTER_SPACING_EM}em`,
                      }}
                    >
                ...
                <g aria-hidden="true">
                  ...
                  {WELLBEING_LABELS.map((a) => (
                    <ArcText
                      key={a.id}
                      arc={a}
                      fontSize={WB_FONT_SIZE}
                      style={{
                        fill: "#42597c",
                        fontWeight: 700,
                        letterSpacing: `${WB_LETTER_SPACING_EM}em`,
                      }}
                    >
                      {text.wellbeingRing}
                    </ArcText>
                ...
                <g
                  style={{
                    filter: "drop-shadow(0 4px 10px rgba(23,59,143,.16))",
                  }}
                >
                  {COMPOSITE_GEOMETRY.map((geom, i) => {
                    ...
                    return (
                      ...
                      <g
                        ...
                        className="group cursor-pointer transition-opacity duration-200 focus:outline-none"
                        style={{ opacity }}
                        onClick={() => toggle(comp.id)}
                        onMouseEnter={() => setHovered(comp.id)}
                        onMouseLeave={() => setHovered(null)}
                      ...
                      >
                        ...
                        group's accessible name, so hide them from readers */}
                        ...
                        <path
                          ...
                          stroke={isActive ? EQI_CORE : "#ffffff"}
                          strokeWidth={isActive ? 3.2 : 1.4}
                        />
                        {/* Keyboard focus ring. Hidden for mouse users — browsers
                        also fire focus on click, and a dashed outline around a
                        wedge you just clicked reads as decoration gone wrong.
                        `:focus-visible` limits it to keyboard navigation. */}
                        <path
                          d={geom.path}
                          aria-hidden="true"
                          fill="none"
                  ...
                  >
                    ...
                    <circle
                      ...
                      stroke="#ffffff"
                      strokeWidth={active === HEART_ID ? 4 : 2}
                    />
                    <circle
                      aria-hidden="true"
                      cx={CENTER.x}
            ...
            {/* Belongs to the wheel above it, so it is kept narrow and centred
                under the wheel — full-width it read as a caption for the whole
                section, which is what the trademark line below actually is. */}
            {tipText !== null && (
              <p className="m-0 mx-auto max-w-xl px-2 pb-1 text-center text-[12.5px] leading-relaxed text-slate-500">
                {tipText}
              </p>
            )}
          </div>
        ...
        >
          ...
          {/* Two columns only while the panel is stacked under the wheel and
              ...
              measure the VIEWPORT, not this element. */}
          <div
          ...
          <div className="mt-6 lg:mt-3">
            <button
              type="button"
              onClick={() => setSelected(defaultSelected ?? null)}
      ...
      <p className="mx-auto mt-8 max-w-2xl border-t border-slate-200 pt-4 text-center text-[11px] leading-relaxed text-slate-400">
        {text.trademark}
      </p>
    </div>
  );
